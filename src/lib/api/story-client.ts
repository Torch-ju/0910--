...
export async function callStoryApi(endpoint:AgentEndpoint,body:AgentRequest):Promise<FrameworkResult|NpcResult|RevisionResult>{
  ...
  try{response=await fetch("/api/story/"+endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body),signal:AbortSignal.timeout(380000)});}
