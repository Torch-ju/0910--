...
const CONTRACT = `Return one JSON object only. Wire Fact is exactly {value,source,evidence}; the application owns status, locked, timestamps, document headers, change logs and growth_arc.user_confirmed. Keep every business field and stable ID. Use concise Chinese, but retain all user information. Initial world timelines contain history/opening only and must include at least one opening. Do not reference unavailable NPC IDs. Character aliases retain user names. Character timeline_event_ids reference accepted world.timeline only, never candidate suggestions. Confirmed 不再自动加入 constraints override old input; confirmed merges satisfy clues through identity/name/aliases.`;
...
export class StoryAgents {
  ...
  async npcs(request: AgentRequest): Promise<NpcResult> {
    const system = systemPrompt("NPC Agent", `{characters: WireCharacterProfiles, timeline_suggestions: WireTimelineEvent[], warnings: string[], assistant_message: string}. Create dynamic characters from user clues and world. Preserve every existing named or unnamed user character and never silently reduce a requested count. If none are supplied, propose needed NPCs.`, { common: WIRE_COMMON_SCHEMA, character: (WIRE_CHARACTERS_SCHEMA as Json).$defs, timeline_event: WIRE_TIMELINE_EVENT_SCHEMA });
