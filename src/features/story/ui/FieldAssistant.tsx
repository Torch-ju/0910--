...
export function FieldAssistant({ document, path, locked }: { document: DocumentKind; path: string; locked: boolean }) {
  ...
  const unavailable = locked || model.busy || !!model.candidate || (model.provider?.remaining ?? 0) <= 0;
  const reason = locked ? "该设定已锁定，请先解锁。" : model.busy ? "当前请求尚未完成。" : model.candidate ? "请先采用或保留当前候选。" : (model.provider?.remaining ?? 0) <= 0 ? "模型额度不足。" : "";
