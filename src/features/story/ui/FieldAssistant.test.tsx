...
describe("FieldAssistant", () => {
  ...
  it.each(["locked", "busy", "candidate", "empty-budget"])("disables requests when %s", (kind) => { const current = model(); if (kind === "busy") current.busy = true; if (kind === "candidate") current.candidate = {} as NonNullable<WorkbenchModel["candidate"]>; if (kind === "empty-budget" && current.provider) current.provider.remaining = 0; mount(current, actions(), kind === "locked"); expect((screen.getByRole("button", { name: "AI建议" }) as HTMLButtonElement).disabled).toBe(true); });
});
