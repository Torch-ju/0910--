...
export function ConversationWindow({session,conversation,busy,onSend}:{session:StorySession;conversation:Conversation;busy:boolean;onSend:(command:TurnCommand)=>void}) {
  ...
  return <><section className="wa-dialogue"><b>正文已暂停 · 对话进行中</b><p>你扮演 {conversation.player.name}，正在与 {conversation.speaker.speaker_name}交流。</p><button onClick={()=>setVisible(true)}>打开NPC对话窗口</button></section>
    <dialog ref={dialog} className="wa-conversation" aria-labelledby="conversation-title" onCancel={()=>setVisible(false)} onClose={()=>setVisible(false)}>
      ...
      <div className="wa-chat-log" role="log" aria-label="角色对话记录" aria-live="polite">{conversation.messages.map((message,index)=><article key={index} data-role={message.role}><b>{message.role==='user'?'你 · ':''}{message.name}</b><p>{message.text}</p></article>)}</div>
      ...
      <form onSubmit={event=>{event.preventDefault();send('reply');}}><label htmlFor="npc-reply">你的回应</label><textarea id="npc-reply" rows={3} maxLength={12000} value={draft} onChange={event=>{setDraft(event.target.value);localStorage.setItem(key,event.target.value);}} placeholder="以主角身份说话，或用括号描述行动……"/>
      <div className="wa-actions"><button type="submit" disabled={busy || !!pending || !draft.trim()}>发送</button><button type="button" disabled={busy || !!pending} onClick={()=>send('finish')}>结束对话并继续正文</button></div></form>
