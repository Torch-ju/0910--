...
export function NarrativeCompose({ session, task, busy, running, waiting, unfinished, lastRun, conversation, continuation, dialogue, input, onInput, close, onClose, onSubmit, onCancelTask, onToggleWaiting, onAbandon }: NarrativeComposeProps) {
  ...
  return <section className="wa-compose">
    ...
    {((unfinished && unfinished.status !== "waiting_dialogue") || task?.status === "interrupted") && <div className="wa-recovery"><p>上次的输入：{unfinished?.input ?? task?.command.input}</p><button disabled={busy || running} onClick={resume}>接着写上次没写完的（会消耗额度）</button>{unfinished && <button disabled={busy || running} onClick={onAbandon}>放弃这一段</button>}</div>}
    ...
    {continuation && <div className="wa-recovery"><p>对话结束了，可以接着写正文。</p><button disabled={busy || running} onClick={() => { const command = continuation.exchanges.at(-1)?.command; if (command) onSubmit({ ...command, retry_failed: true }); }}>继续写正文</button></div>}
