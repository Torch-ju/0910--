...
const labels = { roles: "角色", narrator: "旁白", transcription: "小说转写", memory_extraction: "记忆抽取", memory_update: "记忆更新", summary: "摘要", chapter: "章节" };
...
export function WritingApp(props: WorkbenchProps) {
  ...
  const open = useCallback(async (id: string) => {
    ...
    if (pending) {
      ...
      if (next.runs.some(run => run.operation_id === saved.command.operation_id && ["succeeded", "abandoned"].includes(run.status))) { localStorage.removeItem(taskKey(id)); if (localStorage.getItem(draftKey(id)) === saved.command.input) { localStorage.removeItem(draftKey(id)); setInput(""); } }
      else setTask(saved);
    }
  }, []);
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (current.status === "done" || current.status === "interrupted" || current.status === "cancelled") {
          const done = next.runs.find(run => run.operation_id === current.command.operation_id)?.status === "succeeded";
  ...
  const submit = (command?: TurnCommand) => attempt(async () => {
    if (!session) return;
    const value = command ?? { story_id: session.snapshot.world.story_id, operation_id: uid("op"), base_revision: session.revision, input, close_chapter: close };
  ...
  const initialize = () => attempt(async () => {
    if (props.model.candidate) throw Error("请先采用或放弃候选，再确认当前设定。");
    const next = await api<StorySession>("/api/story/main", { action: "initialize", snapshot: props.model.snapshot });
  ...
  const title = session?.snapshot.world.title.value ?? "书中人";
  return <div className="writing-app">
    ...
    {view === "setup" && <><section className="wa-entry"><div><b>让设定走进故事</b><p>确认世界、人物和开局后，就可以写下第一个行动。</p></div><label><input type="checkbox" checked={props.model.autoNpcs} onChange={e => props.actions.setAutoNpcs(e.target.checked)} />采用世界后自动生成 NPC 候选</label><button className="wa-primary" disabled={busy || props.model.busy || !props.model.restored} onClick={() => void initialize()}>开始写作</button></section><StoryWorkbench {...props} /></>}
    ...
    {view === "writing" && session && <main className="wa-workspace"><header className="wa-heading"><div><p className="wa-eyebrow">正在续写 · 第 {session.chapters.length + 1} 章</p><h1>{title}</h1><p>{session.current_time || "开局时刻待推进"} · {session.current_location || "开局地点待推进"}</p><small>设定版本 {session.snapshot.snapshot_revision} · 叙事版本 {session.revision}</small></div><div className="wa-actions"><button onClick={() => void refresh()} disabled={busy}>查询最新状态</button><button onClick={() => download(title + ".md", "# " + title + "\n\n" + session.turns.map(t => t.prose.content).join("\n\n"))}>导出全文</button><button onClick={() => void attempt(async () => download(title + ".backup.json", JSON.stringify(await api("/api/story/library?view=backup&story_id=" + storyId), null, 2), "application/json"))}>完整备份</button></div></header>
      ...
      {panel === "reading" && <div className="wa-reading"><section aria-label="小说正文" className="wa-prose">{session.turns.length === 0 && <div className="wa-empty"><h2>故事将从这里展开</h2><p>写下你的行动或对白，留一点空间给世界回应。</p></div>}{session.turns.map((turn, index) => <article key={turn.id} id={turn.id}><small>第 {turn.chapter} 章 · 片段 {index + 1}</small><p>{turn.prose.content}</p></article>)}</section>
      <section className="wa-compose">{lastRun && <div className="wa-progress" aria-live="polite"><b>{lastRun.status === "succeeded" ? "本轮已完成" : lastRun.status === "abandoned" ? "此轮已放弃，原输入留档" : "本轮生成状态"}</b>{lastRun.error && <p role="alert">{lastRun.error}</p>}<ol>{STEP_ORDER.map(name => <li key={name} data-state={lastRun.steps[name]?.status ?? "pending"}>{labels[name]}：{lastRun.steps[name] ? statuses[lastRun.steps[name]!.status] : "等待"}{lastRun.steps[name]?.error && <p>{lastRun.steps[name]!.error}</p>}</li>)}</ol></div>}
      {(unfinished || task?.status === "interrupted") && <div className="wa-recovery"><p>保留的输入：{unfinished?.input ?? task?.command.input}</p><button disabled={busy || running} onClick={() => {
        if (!window.confirm("仅重试未完成步骤；结果不明的模型请求可能重复计费。确认恢复？")) return;
        const command = unfinished ? { story_id: storyId!, operation_id: unfinished.operation_id, base_revision: unfinished.base_revision, input: unfinished.input, close_chapter: unfinished.close_chapter, retry_failed: true } : { ...task!.command, retry_failed: true };
        void submit(command);
      ...
      <form onSubmit={e => { e.preventDefault(); if (!busy && !running && !unfinished && input.trim()) void submit(); }}><label htmlFor="narrative-input">你的行动或对白</label><textarea id="narrative-input" rows={5} maxLength={12000} value={input} onChange={e => editInput(e.target.value)} onKeyDown={e => { if ((e.ctrlKey || e.metaKey) && e.key === "Enter" && !e.nativeEvent.isComposing) { e.preventDefault(); if (!busy && !running && !unfinished && input.trim()) void submit(); } }} placeholder="我走近柜台，低声询问那封信的来历……" /><div className="wa-actions"><label><input type="checkbox" checked={close} onChange={e => setClose(e.target.checked)} />本轮完成后结束当前章</label><small>{input.length} / 12000 · Ctrl / ⌘ + Enter 提交</small><button className="wa-primary" disabled={busy || running || !!unfinished || !input.trim()}>推进故事</button></div></form></section></div>}
