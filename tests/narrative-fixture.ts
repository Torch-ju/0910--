...
import { PROMPTS } from "@/lib/orchestration/prompts";
...
export class FixtureModel implements JsonAgentClient {
  ...
  async generate(_id: string, system: string): Promise<unknown> {
    this.calls++;
    const step = Object.entries(PROMPTS).find(([, prompt]) => prompt === system)?.[0] ?? "memory";
    ...
    if (step === "transcription") return { content: "雨敲着窗。沈砚抬头望向门口，没有作声。林青站在门边，柜台后仍留着一盏灯。", current_time: "二更", current_location: "客栈", new_facts: [], timeline_updates: [], foreshadowing: ["柜台后留下的灯"], chapter_end_hook: "灯影晃动。" };
