> 范围说明（2026-09-12）：本文保留为第一层详细规范。完整产品范围已由用户明确扩展，见 [完整 PRD](../../PRD.md) 和 [主 Agent 调用规则](../../MAIN_AGENT_RULES.md)。旧文中“后续不做”仅描述原第一层边界，不否定新增编排任务。
...
### 8. 极简主对话入口（2026-09-13）

首页是一个对话页，只保留一条消息流和一个输入框：没有故事时输入的是故事想法，主 Agent 依次生成世界、人物与开篇；有故事时输入即续写要求，NPC 对话停点也在这条流里出现。世界、人物、时间线不在这里平铺，只显示一张摘要卡片，细节到“故事设定”工作台编辑。写作台、我的作品、故事设定是并列页面，正文区与输入区在对话页和写作台之间共用同一实现。
